...
- Management of the subscripition lifecycle using Stripe webhooks and the _worker_ component.
...
A pricing plan in LetsGo declaratively describes the subscription plans you want to present your users with. Think of a "plan" as one column on your `/pricing` page, and the "pricing plan" as the collection of those plans:
...
1. **Free plans**. These plans are automatically handled by LetsGo and do not require customer's payment. Customers may simply switch to this plan. Any _freemium_ plan falls in this category, and it is a natural plan to start newly signed customers on. You would usually have just one such plan.
...
1. **Custom plans**. This is a plan that requires customer's contact with your sales team. In LetsGo, a customer choosing this plan is redirected to the contact form they are asked to fill out. Handling of that plan is not automated by LetsGo in other way.

For the plans handled by Stripe, a LetsGo plan corresponds to the concept of a [Stripe price](https://stripe.com/docs/products-prices/how-products-and-prices-work) in that it implies a choice a product and a specific way of billing for this product. For example, a SaaS subscription of the same product that can be purchased with either an annual or monthly billing cycle is modeled in LetsGo as two separate LetsGo plans, while in Stripe it is modeled as a single product with two prices.

### Pricing plan definition

The pricing plan of your app is defined as part of the `@letsgo/pricing` package in [packages/pricing/src/index.ts](../../packages/pricing/src/index.ts):
...
- `name` is the name of the plan to be used in various places of the UI.
- `description` is a brief description to be used in the various places of the UI.
...
- `active` indicates if the plan is active or not. Only active plans are offered to customers who are just signing up or switching from another plan. If at some point you want to discontine a specific plan, never remove it from the collection but instead mark it `active = false`. You need to keep the discontinued plans around because you may have existing customers who signed up to it and are billed for it by Stripe. You can only physically remove a plan from the collection if you are sure there are no existing customers signed up to it, which is dependent on your treatment of existing customers when your pricing model changes.
- `usesStripe` indicates this is a paid plan that should be handled by Stripe. LetsGo provides a lot of the scaffolding for managing the lifecycle of a Stripe subscription for such plans. More on this below.
- `actionUrl` is used to tell custom plans where to redirect the user who chooses this plan on the pricing page. By default it is the contact form.
...
This the meaning of individual properties of `SubscriptionPlan`:

- `planId` the `planId` of the pricing plan the tenant is assigned to at present.
- `stripeCustomerId` is the customer Id of a Stripe customer. It is present if the tenant is currently on a Stripe plan, or was on a Strip plan at one point in the past.
...
- `stripeSubscription.status` the standing of the Stripe subscription. The value of `active` means the customer is in good standing (the payments are current). See [other possible states](https://stripe.com/docs/billing/subscriptions/overview#subscription-statuses).
...
In the _web_ component, you can access the list of tenants the logged in user has access to as well as the current tenant using the [`useTenant` hook](./develop-the-frontend.md#get-tenants-and-current-tenant). In the _API_ and _worker_ components, you can use the `getTenant` function exposed by the [@letsgo/tenant](../reference/letsgo-tenant.md) package to access a specific tenant. When calling the _API_, you can use the `GET /v1/me` endpoint to find out the list of tenants the caller has access to, based on the identity information from their access token.
